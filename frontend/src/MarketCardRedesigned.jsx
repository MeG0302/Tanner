...
// --- Semicircular Gauge Component (50% smaller) ---
function SemicircularGauge({ percentage, theme = 'dark' }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-1">
      <svg width="36" height="21" viewBox="0 0 120 70">
        {/* Background arc */}
        <path
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2 mb-2">
        ...
        />
        <h3 className={`text-xs font-medium line-clamp-2 leading-tight flex-1 ${textPrimary}`}>
        ...
        {isBinary ? (
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex justify-between w-full px-4 mb-2">
              <span className={`text-xs ${textSecondary}`}>{(yesOutcome.price * 100).toFixed(1)}¢</span>
              <span className={`text-xs ${textSecondary}`}>{(noOutcome.price * 100).toFixed(1)}¢</span>
            </div>
            
            {/* Yes/No Buttons (50% smaller) */}
            <div className="flex gap-2 w-full">
              <button 
                onClick={(e) => handleTradeClick(e, yesOutcome, 'YES')}
                className="flex-1 py-1 text-white rounded-lg text-xs font-semibold transition-colors"
                style={{ backgroundColor: '#11823b' }}
                onMouseEnter={(e) => e.target.style.backgroundColor = '#0d6b2f'}
                onMouseLeave={(e) => e.target.style.backgroundColor = '#11823b'}
              >
                Yes
              </button>
              <button 
                ...
                className="flex-1 py-1 text-white rounded-lg text-xs font-semibold transition-colors"
                style={{ backgroundColor: '#923c29' }}
                onMouseEnter={(e) => e.target.style.backgroundColor = '#7a3222'}
                onMouseLeave={(e) => e.target.style.backgroundColor = '#923c29'}
              >
                No
              </button>
            </div>
            ...
            {outcomes.slice(0, 3).map((outcome, idx) => (
              <div key={idx} className="flex items-center justify-between">
                <div className="flex items-center gap-2 flex-1">
                  <span className={`text-xs ${textPrimary} truncate max-w-[120px]`}>{outcome.name}</span>
                  <span className={`text-xs font-semibold ${textPrimary}`}>{(outcome.price * 100).toFixed(0)}%</span>
                </div>
                <div className="flex gap-1.5">
                  <button 
                    onClick={(e) => handleTradeClick(e, outcome, 'YES')}
                    className="px-2 py-0.5 text-white rounded text-xs font-medium transition-colors"
                    style={{ backgroundColor: '#11823b' }}
                    onMouseEnter={(e) => e.target.style.backgroundColor = '#0d6b2f'}
                    onMouseLeave={(e) => e.target.style.backgroundColor = '#11823b'}
                  >
