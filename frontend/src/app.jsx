...
export const fetchMarketsByCategory = async (category, setToastMessage) => {
  ...
  const API_URL = `http://92.246.141.205:3001/api/markets/${category}`;
...
function MarketCard({ market, onMarketClick }) {
  ...
  if (outcomes.length === 0) {
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 h-auto">
  ...
  return (
    <div
      className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer p-4 h-auto"
