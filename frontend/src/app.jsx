...
import useStalenessWarning from './useStalenessWarning.jsx';
  ...
  return (
    <main className="flex-1 overflow-y-auto p-8">
      ...
      <div className="flex items-center space-x-3 mb-6">
        ...
        />
        <h1 className="text-3xl font-bold text-white">{market.title || market.question}</h1>
      </div>

      {/* Platform Comparison - Show if market is unified */}
      {market.platforms && Object.keys(market.platforms).length > 1 && (
        <div className="mb-6">
      ...
      {/* Arbitrage Alert - Show if opportunity exists */}
      {market.arbitrage_opportunity && market.arbitrage_opportunity.exists && (
        <div className="mb-6">
          <ArbitrageAlert 
            opportunities={[market.arbitrage_opportunity]}
            theme="dark"
          />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          {/* Price Chart - Use MultiPlatformChart for unified markets */}
          <div className="bg-gray-950 border border-gray-800 rounded-lg p-6">
            ...
            {market.platforms && Object.keys(market.platforms).length > 1 ? (
              ...
              <SimulatedPriceChart data={generateChartData()} />
            )}
          </div>

          {/* Orderbook Comparison - Show for unified markets */}
          {market.platforms && Object.keys(market.platforms).length > 1 && (
            <div className="bg-gray-950 border border-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-semibold text-white mb-4 flex items-center gap-2">
    ...
    <main className="flex-1 overflow-y-auto p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {showUnifiedView ? (
          ...
          filteredUnifiedMarkets.length > 0 ? (
            filteredUnifiedMarkets.map(market => (
              <UnifiedMarketCard
                ...
                onMarketClick={() => onUnifiedMarketClick(market)}
          ...
          // Standard Markets View - Auto-detect unified markets
          filteredMarkets.length > 0 ? (
            filteredMarkets.map(market => {
              // Check if this market has multi-platform data (is a unified market)
              const isUnifiedMarket = market.platforms && 
                                     typeof market.platforms === 'object' && 
                                     Object.keys(market.platforms).length > 1;
              
              // Render UnifiedMarketCard for multi-platform markets, MarketCard for single-platform
              if (isUnifiedMarket) {
                return (
                  <UnifiedMarketCard
                    key={market.unified_id || market.id}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Store in a ref so other effects can access it immediately
    window.__appIdValue = appIdValue;
  ...
  useEffect(() => {
    ...
    const userDocRef = window.firebase.firestore.doc(dbInstance, "artifacts", window.__appIdValue || appId, "users", currentUserId, "portfolio", "data");
  ...
  useEffect(() => {
    ...
    // Also fetch unified markets on initial load
    loadUnifiedMarkets();
  }, []);

  // --- Unified Market Data Fetching Function ---
  const loadUnifiedMarkets = async () => {
    try {
      console.log("Fetching unified markets from /api/unified-markets...");
      const response = await fetch('/api/unified-markets');
      ...
      const data = await response.json();
      console.log("Successfully fetched unified markets:", data);
      
      if (Array.isArray(data)) {
        setUnifiedMarkets(data);
      } else if (data.markets && Array.isArray(data.markets)) {
        // Handle case where data is wrapped in an object
        setUnifiedMarkets(data.markets);
        
        // Also update platform health if provided
        if (data.platformHealth) {
          setPlatformHealth(data.platformHealth);
        }
      } else {
        console.error("Received non-array unified market data:", data);
        setUnifiedMarkets([]);
      }
    } catch (error) {
      console.error("Failed to fetch unified markets:", error);
      handleAddNotification("Could not load unified market data. Using fallback.");
      setToastMessage("Unified markets unavailable");
      setUnifiedMarkets([]);
    }
  };
  ...
  const fetchUnifiedMarkets = async (category = 'All') => {
    try {
      ...
      // Use the main endpoint, optionally with category filter
      const endpoint = category === 'All' 
        ? '/api/unified-markets' 
        : `/api/unified-markets?category=${category}`;
      ...
      if (Array.isArray(data)) {
        ...
        // Update platform health if provided
        if (data.platformHealth) {
          setPlatformHealth(data.platformHealth);
        }
      }
      
      console.log(`Fetched ${marketsData.length} unified markets`);
      setUnifiedMarkets(marketsData);
      return marketsData;
    } catch (error) {
      console.error('Error fetching unified markets:', error);
      handleAddNotification('Failed to load unified markets');
      setToastMessage('Failed to load unified markets');
      
      // Don't clear existing data on error, just return empty array
      return [];
    }
  };
  ...
  const fetchArbitrageOpportunities = async () => {
    try {
      ...
      handleAddNotification('Could not load arbitrage opportunities');
  ...
  const fetchPlatformHealth = async () => {
    try {
      ...
      // Set default healthy status for all platforms on error
      const defaultHealth = {
        polymarket: { status: 'unknown', lastUpdate: null },
        kalshi: { status: 'unknown', lastUpdate: null },
        limitless: { status: 'unknown', lastUpdate: null }
      };
      setPlatformHealth(defaultHealth);
    ...
    if (showUnifiedView) {
      ...
      const fetchAllUnifiedData = async () => {
        try {
          ...
          console.log('All unified market data fetched successfully');
        } catch (error) {
          console.error('Error fetching unified market data:', error);
          handleAddNotification('Some unified market data could not be loaded');
        }
      };
