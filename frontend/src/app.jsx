...
const getLogo = (platform) => {
  switch (platform) {
    ...
    case 'Polymarket':
      return "https://polymarket.com/favicon.ico"; // Real Polymarket icon
    case 'Kalshi':
      ...
      <div className="flex items-start gap-6 mb-6">
        ...
        <div className="flex-1">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-400">Historic Volume:</span>
              <span className="text-lg font-semibold text-white">{formattedVolume}</span>
            </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-4">
            ...
            {/* Theme Toggle - Rectangle with Moon icon only */}
            <button
              onClick={onThemeToggle}
              className={`px-3 py-1.5 rounded-md transition-colors duration-300 focus:outline-none focus:ring-2 ${
                theme === 'dark' 
...
function TickerTape({ newsItems, theme = 'dark' }) {
  return (
    <div className={`border-b h-12 flex items-center overflow-hidden ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-green-50/30 border-green-500'}`}>
      <div className="flex animate-marquee whitespace-nowrap">
        {newsItems.concat(newsItems).map((item, index) => (
          <div key={index} className="flex items-center mx-6">
            ...
            />
            <span className={`text-sm ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>{item.text}</span>
          </div>
        ))}
      </div>
    </div>
...
function MarketCard({ market, onMarketClick, theme = 'dark' }) {
  ...
  if (outcomes.length === 0) {
    return (
      <div className={`rounded-lg shadow-sm p-4 h-auto ${theme === 'dark' ? 'bg-[#1A2332] border border-gray-700' : 'bg-white border border-gray-200'}`}>
        <span className={`text-xs uppercase ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>{market.category}</span>
        <h3 className={`text-sm font-medium mt-2 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{market.shortTitle || market.title}</h3>
        <span className={`text-xs ${theme === 'dark' ? 'text-gray-500' : 'text-gray-400'}`}>Loading...</span>
      </div>
    );
  }
  ...
  return (
    <div
      className={`rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer p-4 h-auto ${theme === 'dark' ? 'bg-[#1A2332] border border-gray-700 hover:border-blue-500/50' : 'bg-white border border-green-500 hover:border-green-600'}`}
    ...
    >
      ...
      <div className="flex items-start gap-2 mb-3">
        ...
        />
        <h3 className={`text-sm font-medium line-clamp-2 leading-tight flex-1 ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-100">
        <div className="flex flex-col">
          <span className="text-xs text-gray-400">Historic Volume</span>
          <span className="text-sm font-semibold text-gray-700">{formattedVolume}</span>
        </div>
        ...
        <div className="flex-1 w-full md:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-10 pr-4 py-2 rounded-lg border focus:outline-none focus:ring-2 ${theme === 'dark' ? 'bg-gray-900 text-white border-gray-700 focus:ring-blue-500' : 'bg-white text-gray-900 border-gray-300 focus:ring-green-500'}`}
            />
          </div>
        </div>
        <div className={`flex space-x-2 p-1 rounded-lg overflow-x-auto ${theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100'}`}>
          {categories.map(cat => (
            <button
              key={cat}
              ...
              className={`flex-shrink-0 px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${
                activeCategory === cat
                  ? (theme === 'dark' ? 'bg-blue-600 text-white' : 'bg-green-500 text-white')
