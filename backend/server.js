...
// --- API Endpoints ---
const API_ENDPOINTS = {
  // Polymarket Gamma API - Fetch ALL active markets (no limit)
  POLYMARKET: 'https://gamma-api.polymarket.com/markets?active=true&closed=false',
  // Polymarket Trending - ALL active markets sorted by volume (matches Polymarket.com trending)
...
async function fetchPolymarketData() {
  try {
    ...
    console.log(`Filtered to ${normalized.length} open markets (removed closed/resolved)`);
...
async function getMarketsByCategory(category) {
  ...
  try {
    ...
    const allMarkets = await fetchPolymarketData();
...
app.get('/api/markets/trending', async (req, res) => {
  ...
  try {
    ...
    console.log(`[API] Fetched ${marketArray.length} trending markets from API`);
    
    // Normalize ALL active markets (no limit)
    ...
    // Cache the results
    cacheManager.setMetadata('trending', normalized);
    cacheManager.trackAccess('trending');
    
    const duration = Date.now() - startTime;
    console.log(`[API] Returning ${normalized.length} trending markets in ${duration}ms (Cache: MISS)`);
    res.json(normalized);
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`[Server] Smart caching enabled with 2-tier system`);
  console.log(`[Server] Cache stats available at: http://localhost:${PORT}/api/stats`);
});
