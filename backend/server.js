...
// --- API Endpoints ---
const API_ENDPOINTS = {
  // Polymarket CLOB API - Official public endpoint (same as monitorthesituation.kl uses)
...
async function fetchPolymarketData() {
  try {
    ...
    // Filter for OPEN markets only (not closed)
    const activeMarkets = marketArray.filter(m => {
      ...
      if (m.accepting_orders !== true) return false;
      
      return true;
    });
    
    console.log(`Filtered to ${activeMarkets.length} open markets`);
