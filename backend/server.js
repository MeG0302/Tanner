...
async function fetchKalshiData() {
  try {
    ...
    // Filter for quality: Skip multi-leg parlays
    const qualityMarkets = marketsData.markets.filter(market => {
      const title = market.subtitle || market.title || '';
      
      // Skip multi-leg parlays (they have many commas)
      if (title.split(',').length > 3) {
        return false;
      }
      
      // Skip if title starts with "yes " or "no " (multi-leg format)
      if (/^(yes|no)\s+/i.test(title)) {
        return false;
      }
      
      // Relaxed volume filter - just need some activity
