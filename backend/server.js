...
// --- API Endpoints ---
const API_ENDPOINTS = {
  // Polymarket Gamma API - Returns active markets with volume data
  POLYMARKET: 'https://gamma-api.polymarket.com/markets?limit=100&active=true&closed=false',
  // Polymarket Trending - Same endpoint Polymarket.com uses for trending tab
  POLYMARKET_TRENDING: 'https://gamma-api.polymarket.com/markets?limit=100&active=true&closed=false&order=volume24hr',
...
async function fetchPolymarketData() {
  try {
    ...
    // CLOB API returns array directly
    const marketArray = Array.isArray(data) ? data : data.data;

    if (!Array.isArray(marketArray)) {
      console.error('Invalid Polymarket response structure. Expected array.');
      throw new Error('Invalid Polymarket structure');
    }
    
    console.log(`Fetched ${marketArray.length} markets from Polymarket`);
    
    // Gamma API already filters for active markets, just use them all
    const activeMarkets = marketArray;
    
    console.log(`Using ${activeMarkets.length} active markets`);
    
    // Debug: Log first market to understand structure
    if (activeMarkets.length > 0) {
      console.log('\n=== SAMPLE POLYMARKET CLOB MARKET ===');
      const sample = activeMarkets[0];
      console.log(JSON.stringify(sample, null, 2).substring(0, 500));
      console.log('=== END SAMPLE ===\n');
    }
    
    // Normalize and sort by volume
    const normalized = activeMarkets
      .map(normalizePolymarket)
      .filter(Boolean)
      .sort((a, b) => b.volume_24h - a.volume_24h);
    
    console.log(`Normalized ${normalized.length} Polymarket markets`);
    
    return normalized.slice(0, 100);
    
  } catch (error) {
    console.error('Failed to fetch from Polymarket CLOB:', error.message);
    return [];
  }
}
...
async function getAllMarkets() {
  ...
  console.log('Fetching fresh market data from Polymarket...');
  
  // Fetch only from Polymarket for now
  const results = await Promise.allSettled([
    fetchPolymarketData(),
    // fetchKalshiData(), // Disabled for now
    // fetchLimitlessData(), // Disabled
  ]);
...
// --- NEW: Get trending markets ---
app.get('/api/markets/trending', async (req, res) => {
  console.log('Received request for trending markets');
  
  try {
    // Fetch trending markets directly from Polymarket
    console.log('Fetching trending markets from Polymarket...');
    const data = await nativeFetch(API_ENDPOINTS.POLYMARKET_TRENDING);
    const marketArray = Array.isArray(data) ? data : data.data;
    ...
    console.log(`Fetched ${marketArray.length} trending markets`);
    
    // Normalize and return
    const normalized = marketArray
      .map(normalizePolymarket)
      .filter(Boolean)
      .slice(0, 25);
    
    console.log(`Returning ${normalized.length} trending markets`);
    res.json(normalized);
...
// --- Get markets by category (top 25) ---
