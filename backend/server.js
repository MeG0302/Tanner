...
/**
 * Fetches market data from Kalshi using Events API (better quality)
 */
async function fetchKalshiData() {
  try {
    console.log('Fetching Kalshi data using Events API...');
    
    // 1. Fetch events (better structure than markets)
    const eventsData = await nativeFetch(API_ENDPOINTS.KALSHI_EVENTS);

    if (!eventsData || !Array.isArray(eventsData.events)) {
        console.error('Invalid Kalshi Events response structure. Expected { events: [...] }');
        throw new Error('Invalid Kalshi Events structure');
    }

    console.log(`Fetched ${eventsData.events.length} events from Kalshi`);

    // 2. Filter for quality: Only simple, clear events
    const qualityEvents = eventsData.events.filter(event => {
      // Skip multi-leg parlays (they have commas in title)
      if (event.title && event.title.split(',').length > 3) {
        return false;
      }
      
      // Skip if title starts with "yes " or "no " (multi-leg format)
      if (event.title && /^(yes|no)\s+/i.test(event.title)) {
        return false;
      }
      
      // Must have volume
      if (!event.volume || event.volume < 100) {
        return false;
      }
    ...
    console.log(`Filtered to ${qualityEvents.length} quality events`);
    ...
    return qualityEvents
      ...
      .slice(0, 50); // Get top 50 for better category distribution
