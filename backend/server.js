...
async function fetchPolymarketData() {
  try {
    console.log('Fetching from Polymarket CLOB API (official public endpoint)...');
    const data = await nativeFetch(API_ENDPOINTS.POLYMARKET_CLOB);
    ...
    console.log(`Fetched ${marketArray.length} markets from Polymarket CLOB`);
    
    // Filter for active markets with volume
    const activeMarkets = marketArray.filter(m => {
      const volume = parseFloat(m.volume_24h || m.volume || 0);
      const isActive = m.active !== false && m.closed !== true;
      return isActive && volume > 100; // At least $100 volume
    });
