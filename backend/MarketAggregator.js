...
class MarketAggregator {
  ...
  async getUnifiedMarkets(category) {
    try {
      ...
      // Check cache first - get all cached unified markets
      const allCached = this.cache.getAllUnifiedMarkets();
      if (allCached && allCached.length > 0) {
        console.log(`[MarketAggregator] Cache hit - filtering ${allCached.length} markets for ${category}`);
      ...
      // Cache each unified market individually
      filtered.forEach(market => {
        this.cache.setUnifiedMarket(market.unified_id, market);
      });
